...
import { Table, Pagination, TableBody, TableColumn, TableHeader, TableRow, TableCell, Spinner } from "@nextui-org/react";
...
const PatientJourneyDetails = ({ isVisible, setVisibility, selectdataPatientJourney, }) => {
  ...
  const refreshUsersWithoutSearch = async () => {
    setLoading(true);
    try {
      const response = await newRequest.get(`/api/v1/journeys/patient/${patientsDate?.id || ""}`);
